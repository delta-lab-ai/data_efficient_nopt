...
class Trainer:
    ...
    def initialize_data(self, params):
        ...
        if self.params.model_type == 'fno':
            ...
            params.masking = ((params.n_steps, params.input_size//params.patch_size, params.input_size//params.patch_size), params.mask_ratio)
    ...
    def initialize_model(self, params):
        if self.params.model_type == 'fno':
            self.model = fno_pretrain(params).to(self.device)
        elif self.params.model_type == 'vmae':
    ...
    def train_one_epoch(self):
        ...
        for batch_idx, data in enumerate(self.train_data_loader):
            ...
            try:
                if len(data) == 3:
                    inp, _, mask = map(lambda x: x.to(self.device), data) 
                    if sum(params.blur) > 0:
                        ...
                        inp_blur = torch.stack(inp_blur, dim=0)
                    else:
                        inp_blur = inp.detach().clone()
                else:
                    inp, _ = map(lambda x: x.to(self.device), data) 
                    mask = None
                    inp_blur = inp.detach().clone()
            except:
            ...
            #         inp_blur = torch.cat([inp_blur, torch.zeros(b, 1, h, w)], dim=1)
            ...
            with amp.autocast(self.params.enable_amp, dtype=self.mp_type):
                ...
                if len(inp.shape) == 5:
                    ...
                    labels = rearrange(
                        ...
                        h=inp.shape[3]//self.model.patch_size, w=inp.shape[4]//self.model.patch_size,
                        p1=self.model.patch_size, p2=self.model.patch_size
                    )
                    if mask is not None:
                        ...
                        if mask.sum() == 0:
                            ...
                            labels = labels[mask]
                    labels = labels.reshape(inp.shape[1], -1, inp.shape[2]*self.model.patch_size**2)
                    ...
                    raw_loss = (residuals).pow(2).mean(spatial_dims, keepdim=True) / inp_norm
    ...
    def single_dset_val(self, subset, logs, cutoff=40):
        ...
        for _, data in enumerate(temp_loader):
            ...
            nmse = (residuals.pow(2).mean(spatial_dims, keepdim=True) 
                / (1e-7 + inp.pow(2).mean(spatial_dims, keepdim=True))).sqrt()
            ...
            logs['valid_l2'] = (logs.get('valid_l2', 0)*(count-1) + l2_err(output, inp, spatial_dims))/count
