...
class BaseHDF5DirectoryDataset(Dataset):
    ...
    def __init__(self, path, include_string='', n_steps=1, dt=1, split='train', 
                 train_val_test=None, subname=None, extra_specific=False, rollout=1):
        super().__init__()
        self.path = path
        self.split = split
    ...
    def _get_directory_stats(self, path):
                ...
                try:
                    with h5py.File(file, 'r') as _f:
                        ...
                        if self.split_level == 'sample':
                            ...
                            sample_per_part = np.ceil(np.absolute(np.array(self.train_val_test)*samples)).astype(int)
                            ...
                            if self.train_val_test[0] >= 0:
                                ...
                                if partition == 0:
                                    self.split_offsets.append(self.file_steps[-1]*(1 - sum(sample_per_part)))
                                else:
                                    self.split_offsets.append(self.file_steps[-1]*(1-sum(sample_per_part[partition:])))
        ...
        # Figure out our split offset - by sample
            if self.train_val_test is None:
                ...
                if self.train_val_test[1]*total_samples < 1 or self.train_val_test[2]*total_samples < 1:
                    ideal_split_offsets = [self.train_val_test[i]*total_samples for i in range(3)] # TODO: how many "files" to use
                    ideal_split_offsets = [int(value) if value >= 1 else value for value in ideal_split_offsets]
                else:
                    ideal_split_offsets = [int(self.train_val_test[i]*total_samples) for i in range(3)] # TODO: how many "files" to use
                ...
                if ideal_split_offsets[0] > 0:
                    ...
                    end_ind = total_samples - round(sum(ideal_split_offsets[1:])) - ideal_split_offsets[0]
                for i in range(self.partition+1):
                    ...
                    for samples, steps in zip(self.file_samples, self.file_steps):
                        ...
                        if run_sum <= ideal_split_offsets[i]:
                            ...
                            end_ind += round(np.abs((run_sum - samples) - ideal_split_offsets[i]) * (steps))
                            break
    ...
    def __getitem__(self, index):
        ...
        nsteps = self.file_nsteps[file_idx] + self.rollout-1 # Number of steps per sample in given file
