import scipy.io
import numpy as np
from .masking_generator import MaskingGenerator, TubeMaskingGenerator
...
from utils.utils import get_grid3d, convert_ic, torch2dgrid
...
class KFDataset(Dataset):
    def __init__(self, paths, 
        ...
        if isinstance(self.masking, float):# and self.masking > 0:
            self.mask_generator = TubeMaskingGenerator((self.a_data.shape[1], self.a_data.shape[2], self.T), self.masking)

    def load(self):
        raw_data = []
        for _path in self.paths:
            _data = np.load(_path, mmap_mode='r')
    ...
    def partition(self, data):
        ...
        # new_data = np.zeros((K * N, T // K, S, S)) # TODO:
        step = T // K
        for i in range(N):
            for j in range(K):
                new_data[i * K + j] = data[i, j * step: (j+1) * step + 1]
                # new_data[i * K + j] = data[i, j * step: (j+1) * step] # TODO:
        return new_data
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(self.masking, float):
            ...
            mask = self.mask_generator().reshape(self.a_data.shape[1], self.a_data.shape[2], self.T, 1)
            return self.data[idx], a_data, mask
        else:
...
class KFDataset_Pretrain(KFDataset):
    def load(self):
        raw_data = []
        for _path in self.paths:
            _data = np.load(_path, mmap_mode='r')
        ...
        a_sub_x = self.raw_res[0] // self.data_res[0]
        # load data
        # data = raw_data[self.offset: self.offset + self.n_samples, ::sub_t, ::sub_x, ::sub_x]
        # divide data
        # end_t = self.raw_res[2] - 1
        ...
        # data = self.partition(data)
        # a_data = raw_data[self.offset: self.offset + self.n_samples, 0:end_t:step, ::a_sub_x, ::a_sub_x]
        a_data = raw_data[self.offset: self.offset + self.n_samples, ::a_sub_x, ::a_sub_x]
        a_data = a_data.reshape(self.n_samples, 1, self.data_res[0], self.data_res[1]) # 2N x 1 x S x S
...
class FNODatasetSingle(Dataset):
    def __init__(self, filename,
        ...
        print(f'num_samples_max:{num_samples_max}')
        
        if pretrain:
            if if_test:
                self.data = self.data[:100] #100
            else:
                self.data = self.data[240:]  #240:
            
            # self.data = self.data[..., 0, :]
            self.data = self.data.permute(0, 3, 1, 2, 4)
            data_shape = list(self.data.shape)[2:]
            data_shape = [-1] + data_shape
            self.data = self.data.reshape(data_shape)#[::10]#[1::101]  #n_sample, nx, ny, nc
            ...
            self.masking_generator = MaskingGenerator(tuple(list(self.data.shape))[1:3], mask_ratio)
            # self.masking_generator = MaskingGenerator(tuple(list(self.data.shape))[2:4], mask_ratio)
        else:
            if if_test:
                self.data = self.data[200:240]
                # self.data = self.data[400:440]
            else:
                assert num_samples_max <= 200
                if num_samples_max < 0:
                    num_samples_max = self.data.shape[0]
                num_samples_max  = min(num_samples_max, self.data.shape[0])
                # sample_idxs = np.linspace(0, 200, num=num_samples_max, dtype=int)
    ...
    def __getitem__(self, idx): 
        if self.pretrain:
            idx_mask = np.stack([self.masking_generator().reshape(self.data.shape[1], self.data.shape[2]),
                                 self.masking_generator().reshape(self.data.shape[1], self.data.shape[2]),
                                 self.masking_generator().reshape(self.data.shape[1], self.data.shape[2]),
                                 self.masking_generator().reshape(self.data.shape[1], self.data.shape[2])], axis=-1)
            self.mask = torch.from_numpy(idx_mask)
            
            concat_data = torch.cat([self.data[idx], self.grid], dim=-1)
            data_blur = concat_data.permute(2, 0, 1)
            sigma = random.uniform(*self.blur)
            # https://github.com/scipy/scipy/blob/v1.11.4/scipy/ndimage/_filters.py#L232
            _kernel = min(int((sigma*4+1)/2)*2+1, (data_blur.shape[-1]//2)*2-1)
            if _kernel >= 2:
                data_blur = functional.gaussian_blur(data_blur, kernel_size=[_kernel, _kernel], sigma=sigma)
            data_blur = data_blur.permute(1, 2, 0)
            return concat_data, data_blur, self.grid, self.mask
        # for d in [self.data[idx,...,:self.initial_step,:], self.data[idx], self.grid]:
            # print('datashape', d.shape)
        return self.data[idx,...,:self.initial_step,:], self.data[idx], self.grid


class PINOPretrainDataset(FNODatasetSingle):
    def __init__(self,
                 if_test=False, 
                 mask_ratio = 0,
                 blur=[0, 0]
        ...
        self.mask_ratio = mask_ratio
        self.blur = blur
        # datapaths =  ['/pscratch/sd/c/chenwy/physics_informed_pretrain/Re100.npy', '/pscratch/sd/c/chenwy/physics_informed_pretrain/Re300.npy', '/pscratch/sd/c/chenwy/physics_informed_pretrain/Re500.npy', '/pscratch/sd/c/chenwy/physics_informed_pretrain/Re800.npy', '/pscratch/sd/c/chenwy/physics_informed_pretrain/Re1000.npy']
        datapaths =  ['/global/homes/c/chenwy/data/NS-Re300_T300_id0.npy']
        raw_data = []
        for _path in datapaths:
            _data = np.load(_path, mmap_mode='r')
            raw_data.append(_data)
            # print(_data.shape)
        ...
        bp()
        if if_test:
            self.data = self.data[51790:, ::2, ::2]
        else:
            self.data = self.data[:51790, ::2, ::2]
        self.data = torch.from_numpy(self.data).to(torch.float32).unsqueeze(1).permute(0, 2, 3, 1)
        self.data = torch.cat([self.data, torch.zeros(self.data.shape)], axis=-1)
        self.S = 128
        self.grid = torch.from_numpy(self.get_2dgrid()).to(torch.float32)
        self.masking_generator = MaskingGenerator(tuple(list(self.data.shape))[1:3], mask_ratio)
        print(self.data.shape)
        print(self.grid.shape)
    ...
    def get_2dgrid(self):
        ...
        '''
        xarr = np.linspace(0, 1, self.S)
        yarr = np.linspace(0, 1, self.S)
